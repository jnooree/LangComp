...
def respond(message, last_response=None):
    ...
    # "be like"
    match = re.match(r".*\b(BE(?:EN)?|[A']M|[I']S|[A']RE|WAS|WERE) LIKE\b.*",
                     message)
    if match:
        return "WHAT RESEMBLEMCE DO YOU SEE"

    # you (are|do)( not|n't)? (.*) me
    ...
    if match:
        ...
        family_match = re.match(r"^((?:FATHER|MOTHER|MOM|DAD|SISTER|BROTHER"
                                r"|WIFE|HUSBAND|SON|DAUGHTER|CHILD(?:REN)?)S?)\b"
                                r"(.*)",
                                trailing)
        if family_match:
            ...
            if family_match.group(2):
                responses.extend([
                    f"WHO ELSE IN YOUR FAMIY{family_match.group(2)}",
...
if __name__ == '__main__':
    ...
    for line in data[::2]:
        line = line.replace("’", "'")
        print(line)
        last_response = respond(line, last_response)
        print(last_response)
